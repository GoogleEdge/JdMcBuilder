...
当前实现的能力探测契约：能力探针必须由用户在测试/备份世界中明确确认，并为 WorldEdit、原生 `/fill`、原生 `/setblock` 分别提供互不重叠的测试范围/点和探针方块。探测先调用 `mcc_session_status`、`mcc_world_state`（可选 `mcc_server_info`）生成目标指纹；session 和 world 响应必须各自至少包含一个应用明确识别的稳定 identity 字段。标准 MCP `result.content[].text` 中的完整 JSON 对象可作为机器身份的最后回退来源；StructuredContent/RawResult 机器字段优先，冲突值 fail-closed，普通人类文本、聊天摘要和通用 observation hash 不得生成目标指纹。空对象、只有未知字段或冲突身份不得生成目标指纹，且必须在任何写入前 fail-closed。目标为真实 MCP HTTP client 时，目标指纹还必须包含 endpoint 与当前 `Mcp-Session-Id` 派生的 transport context；缺少 session ID、session header 在活动连接中轮换或 endpoint/session context 改变时，丢弃证明且不重放写入。然后才按后端独立执行写入，并以 `mcc_world_block_at` 的坐标绑定方块 ID 采样作为当前客户端观察层的验证。WorldEdit 和原生 `/fill` 都在各自的一次写入发送后，对标准化范围的 1/2/4/8 个去重角点进行有界只读轮询，以处理短暂可见性延迟；这只是角点抽样，不是对区域内部全部方块的完整扫描。若发现 `mcc_chunk_status`，每个角点采样前必须先确认目标 chunk `loaded=true` 且 `fullyLoaded=true`；未加载时不调用 `mcc_world_block_at`，只在有界时间内重试 readiness，最终保持不确定。chunk readiness 只说明 MCC 客户端 terrain cache 可观测，不是服务器权威 fresh read；即使 readiness 通过，`mcc_world_block_at` 仍不能单独证明刚刚发送的命令已经在服务器完成。缺少 `mcc_chunk_status` 时可兼容地读取，但必须把 cache freshness 标为未知。`mcc_chat_history` 若存在只能作为可选诊断，不能证明当前命令或目标坐标，聊天/debug 中“成功填充 N 个方块”同样不能证明世界状态。原生 `/setblock` 对每个显式 placement 只发送一次 `/setblock`，并读取同一坐标；首个可解析但不匹配的旧值允许触发有界只读轮询，服务器返回的“更改了位于...的方块”等文本仅作诊断，不能替代采样。只有该后端自己的写入和全部独立采样均成功时，才生成带后端 ID、目标指纹、验证时间和过期时间的 `BackendVerification`；任何权限错误、取消、超时、结果缺失、错误返回坐标、readiness 无效或采样不匹配都不生成证明。探测失败且写入可能已发送时必须提示人工检查探针位置，不得自动重放 `//set`、`/fill`、`/setblock`，偏移坐标或静默切换后端。目标指纹不匹配或过期时，真实施工 gate 继续保持关闭。能力探针证明后端现在可用，但不能证明此前不确定批次当时是否已经完成，也不得修改或清除旧 journal 的不确定状态。
...
重启后只允许从成功批次继续。对于经 `mcc_send_chat` 发送的 WorldEdit `//set`、`/fill` 或 `/setblock`，如果返回结果不确定，整个批次进入 `uncertain`，必须先用与当前 MCP transport session/target 绑定的只读角点采样核验，而不是自动重放；若发现 `mcc_chunk_status`，未完全加载的目标 chunk 只能报告客户端缓存不可观测，不能把默认 Air 当作服务器状态；`/setblock` batch 不得只重放尚未发送的剩余 placement。只要 journal 中存在任何不确定批次，执行器就在选择后端、保存新的 in-flight checkpoint 或调用任何世界工具前全局停止，不能先执行位于该批次之前的 pending batch。
