...
The normal backend preference is WorldEdit for large fills, native `/fill` next, and `mcc_place_block` as a last resort for small explicit placements. WorldEdit commands are sent through `mcc_send_chat`, never through `mcc_run_internal_command`. For the verified target deployment, the app sends one `//pos` command with two comma-separated block vectors, followed by `//set`, for example `//pos 1,64,2 3,65,4` and `//set minecraft:stone`. This uses the WorldEdit command form accepted by the target server; do not replace it with separate `//pos1` and `//pos2` commands. The syntax is deployment-specific and must not be generalized to every MCC, Leaf, Paper, or WorldEdit version. A full world being available does not remove the blueprint bounds, batch, or confirmation guardrails. This command form is covered by offline regression tests but still requires a live target-world probe for permission and mutation verification.
