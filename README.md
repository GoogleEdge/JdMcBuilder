...
The normal backend preference is WorldEdit for large fills, native `/fill` next, and `mcc_place_block` as a last resort for small explicit placements. For the WorldEdit path, the app passes `/pos1`, `/pos2`, and `/set` to `mcc_send_chat`; this is intentional. A player normally types the WorldEdit aliases as `//pos1`, `//pos2`, and `//set`, but MCC consumes the outer command slash when it builds the serverbound command packet, so sending the double-slash form through `mcc_send_chat` would reach the server as a command name beginning with `/`. A full world being available does not remove the blueprint bounds, batch, or confirmation guardrails.
