...
The normal backend preference is WorldEdit for large fills, native `/fill` next, and `mcc_place_block` as a last resort for small explicit placements. For the verified target deployment, the app passes `///pos1`, `///pos2`, and `///set` to `mcc_send_chat`. MCC consumes one outer slash, leaving the double-slash form accepted by this MCC → Leaf → WorldEdit path. Coordinates remain space-separated because that is the exact command shape verified by the live probe, for example `///pos1 1 64 2`. This syntax is deployment-specific and must not be generalized to every MCC, Leaf, Paper, or WorldEdit version. A full world being available does not remove the blueprint bounds, batch, or confirmation guardrails. This command form is covered by offline regression tests but still requires a live target-world probe for permission and mutation verification.
