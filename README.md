...
4. **Probe independently:** Enter three safe, non-overlapping test locations/ranges and a probe block, then click the capability-validation button. The probe writes test blocks, so it requires an explicit confirmation and a recoverable test area. WorldEdit, native `/fill`, and native `/setblock` are verified separately.
...
The normal backend preference is WorldEdit for large fills, native `/fill` next, and native `/setblock` for small explicit placements. WorldEdit and `/setblock` commands are sent through `mcc_send_chat`, never through `mcc_run_internal_command`. For `/setblock`, the app generates exactly one validated command per placement, such as `mcc_send_chat({"text":"/setblock 6 64 1 minecraft:stone"})`; it does not select inventory items, inspect held items, move the player, or require line of sight. The server response, including a message such as `更改了位于6, 64, 1的方块`, is diagnostic only. The app independently calls `mcc_world_block_at` for the same coordinate and compares the canonical block ID; after the single send it may perform bounded, read-only polling when the first sample is a stale pre-write value. Polling never resends `/setblock`, shifts coordinates, or uses chat history as proof. A persistent mismatch, malformed or wrong-coordinate sample, timeout, cancellation, or transport error remains uncertain and is not automatically replayed. For the verified target deployment, the app sends one `//pos` command with two comma-separated block vectors, followed by `//set`, for example `//pos 1,64,2 3,65,4` and `//set minecraft:stone`. This uses the WorldEdit command form accepted by the target server; do not replace it with separate `//pos1` and `//pos2` commands. The syntax is deployment-specific and must not be generalized to every MCC, Leaf, Paper, or WorldEdit version. A full world being available does not remove the blueprint bounds, batch, or confirmation guardrails. These command forms are covered by offline regression tests but still require a live target-world probe for permission and mutation verification.

For native `/fill`, a successful `mcc_send_chat` response or a Minecraft chat/debug line such as “successfully filled 2 blocks” is only a diagnostic observation. The app displays the normalized inclusive range, exact generated command, and deduplicated corner samples, then performs bounded read-only `mcc_world_block_at` polling. It never resends `/fill`, shifts coordinates, or silently falls back after an uncertain write. Any persistent air, partial fill, unreadable sample, wrong returned coordinate, timeout, cancellation, or transport error remains unverified and stops the operation.
